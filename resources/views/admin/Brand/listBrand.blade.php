        ...
        @forelse ($brands as $brand)
            <div class="bg-white rounded-xl shadow-2xl overflow-hidden transition duration-300 ease-in-out hover:shadow-indigo-300/50 hover:shadow-3xl border border-gray-100">
                ...
                <div class="p-6">
                    ...
                    {{-- Brand Name --}}
                    <h2 class="text-2xl font-bold text-gray-900 mb-4 truncate" title="{{ $brand->BrandName }}">
                        {{ $brand->BrandName}}
                    </h2>
                ...
                <div class="p-4 bg-gray-50 border-t flex space-x-3">
                    {{-- Edit Button --}}
                    <a href="{{ route('brands.update.view', $brand->id) }}" 
                    ...
                    <form action="{{ route('brands.delete', $brand->id) }}" method="POST" class="flex-1"
                          onsubmit="return confirm('Are you sure you want to permanently remove the brand: {{ $brand->brand_name }}? This action cannot be undone.');">
                        @csrf
                        <button type="submit" 
